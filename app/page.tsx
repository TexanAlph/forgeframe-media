          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 items-start">
            {PLANS.map((p) => (
              <div key={p.name} className={`ff-reveal ff-card relative rounded-2xl p-7 ff-glass ${p.featured ? "ff-halo border-[#ff7a2f]/50 ff-glow md:-translate-y-3" : ""}`}>
                {p.featured && <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-[#ff7a2f] px-3 py-1 text-[10px] font-black uppercase tracking-widest text-black">Recommended</span>}
